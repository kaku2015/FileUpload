...
public class ApkController {
    ...
    public static boolean uninstall(String packageName, Context context) {
        if (hasRootPerssion()) {
            // 有root权限，利用静默卸载实现
            boolean result = clientUninstall(packageName);
            if (!result) {
                return intentionUninstall(packageName, context);
            }
            return true;
        } else {
            return intentionUninstall(packageName, context);
     ...
     * 意图卸载
     */
    private static boolean intentionUninstall(String packageName, Context context) {
